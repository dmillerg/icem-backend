...
async function recogida() {
  ...
  conexion.query(
    `SELECT * FROM scrap WHERE activo=1`,
    function (error, results, fields) {
      if (results.length > 0) {
        results.forEach((ele) => {
          scrapeItAll(ele).then(function (e) {
            ...
            scrape.forEach((e, index, array) => {
              ...
              conexion.query(
                `INSERT INTO noticias(id, titulo, descripcion, fecha, imagen, enlace, fuente, logo) VALUES (NULL, '${e.titulo}', '${e.descripcion}', '${date}', '${e.imagen}', '${e.enlace}', '${e.fuente}', '${e.logo}')`,
                function (error, resultado, fieldss) {
...
function iniciarScrapping(req, res) {
  conexion.query(
    `SELECT config FROM configuraciones WHERE nombre = 'scrap_time'`,
        ...
        interval = setInterval(() => {
          try {
            recogida().then(() => {
              console.log(
                "SUCCESS",
                "Se han recogido los datos de las paginas correctamente"
              );
            });
          } catch (e) {
            console.log(
              "En estos momentos no se puede acceder por favor intente mas tarde"
            );
          }
        }, result[0].config * 60000);
        return res.status(200).send({
          message: `intervalo creado en ${result[0].config } minutos`,
        });
      }
    }
  );
...
function obtenerScrappingLoop(req, res) {
  return res.status(200).send(interval === 0);
}
