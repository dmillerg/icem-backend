    ...
    conexion.query(
      ...
      function (errr, ress) {
        ...
        if (ress.length > 0) {
          return res
            .status(400)
            .send({ message: "Este usuario ya esta siendo utilizado" });
        } else {
          conexion.query(
            `SELECT * FROM usuarios WHERE correo='${correo}'`,
            function (errrt, resst) {
              if (errrt) {
              ...
              if (resst.length > 0) {
                return res
                  .status(405)
                  .send({ message: "Este correo ya esta siendo utlizado" });
              } else {
                  ...
                  if (err) {
                    ...
                    conexion.query(
                      ...
                      function (error, results, fields) {
                        ...
                        if (results) {
                          console.log(results);
                          ingresarPasswordHistorico(results.insertId, encrypted);
  ...
  if (await login.isLogin(req.query.token)) {
    ...
    conexion.query(
      ...
      function (err, result) {
        ...
        if (result.length > 0) {
          conexion.query(
            `SELECT * FROM usuarios WHERE correo='${correo}' AND id<>${id}`,
            function (ec, suc) {
              console.log("succes", suc);
              if (ec)
                res.status(500).send({
                  message: "error en el servidor en la verificacion del correo",
                });
              if (suc.length > 0)
                res
                  .status(500)
                  .send({ message: "el correo ya esta siendo utilizado" });
              else {
                // Buscamos por id y actualizamos el objeto y devolvemos el objeto actualizado
          ...
          bcrypt.hash(new_password, 10, (err, encrypted) => {
            if (err) {
              ...
              conexion.query(query, function (error, resultado) {
                ...
                if (resultado) {
                  ingresarPasswordHistorico(id_usuario, encrypted);
                  return res.status(200).send({
                    message: "SUCCESS, passsword cambiada correctamente",
                  });
                }
              });
            }
      ...
      if (err) {
        ...
        conexion.query(query, function (error, resultado) {
          ...
          if (resultado) {
            ...
            return res
              .status(200)
              .send({ message: "SUCCESS, passsword cambiada correctamente" });
          }
        });
      }
    });
...
async function changePassword(req, res) {
  console.log(
    await checkearPasswordAnteriores(req.body.id_usuario, req.body.new_password)
  );
  if (
    await checkearPasswordAnteriores(req.body.id_usuario, req.body.new_password)
  ) {
            ...
            if (result.length > 0) {
              if (bcrypt.compareSync(req.body.pass_old, result[0].password)) {
                  ...
                  if (err) {
                    ...
                    conexion.query(query, function (error, resultado) {
                      ...
                      if (resultado) {
                        ingresarPasswordHistorico(id_usuario, encrypted);
                        return res.status(200).send({
                          message: "passsword cambiada correctamente",
                        });
                      }
                    });
                  }
                ...
                return res.status(400).send({
                  message: "ERROR, la contrasenna anterior no es correcta",
                });
              }
            }
          });
    ...
    return res
      .status(400)
      .send({ message: "Ya ha usado esa contraseña, por favor utilice otra." });
  }
}
        ...
        function (err, resul) {
          if (err) {
            return res.status(500).send({
              message: "ERROR",
              error: error,
              sms: "usuario activado pero no pudimos obtener sus datos",
            });
          }
          if (resul) {
            return res.status(200).send({
...
function checkearPasswordAnteriores(id_usuario, pass) {
  return new Promise((resolve, reject) => {
    conexion.query(
      `SELECT * FROM historico_password WHERE user_id = ${id_usuario}`,
        ...
        if (result) {
          if (result.length > 0) {
            ...
            result.forEach((element) => {
              if (bcrypt.compareSync(pass, element.password)) {
                retorna = false;
              }
            });
            return resolve(retorna);
          } else {
            return resolve(true);
          }
...
function ingresarPasswordHistorico(id_usuario, password) {
  return new Promise((resolve, reject) => {
    conexion.query(
      `SELECT * FROM historico_password WHERE user_id = ${id_usuario}`,
      function (error, result) {
        ...
        if (result) {
          if (result.length > 9) {
            conexion.query(
              `DELETE FROM historico_password WHERE user_id = ${id_usuario} AND password='${result[0].password}'`
            );
          }
          conexion.query(
            `INSERT INTO historico_password(user_id, password, fecha) VALUES ("${id_usuario}","${password}", NOW())`,
